...
import cz.vutbr.fit.communication.command.builder.ICommandBuilder;
...
public class Consumer<K, V> extends AbstractConsumer<K, V> {
    ...
    public void consume() {
        while (true) {
            long timeout = Long.parseLong(cz.vutbr.fit.properties.Properties.getInstance().loadProperty("poll.timeout"));
