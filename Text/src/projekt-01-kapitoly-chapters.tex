...
\noindent HDFS je založen na \texttt{GFS} (Google File System) a lze jej provozovat na rozsáhlých seskupeních počítačů. Používá architekturu typu \texttt{master/slave}. Master se skládá z jednoho uzlu nazývaného \texttt{NameNode}, který se stará o správu metadat souborového systému. Uzlů typu slave může být více, jsou nazývány datové uzly (angl. \texttt{DataNodes}), a ukládají data. Soubor je rozdělen do několika bloků (typicky po 64 nebo 128 MB), a každý blok souboru je nezávisle replikován v několika datových uzlech slave. Bloky jsou uloženy v lokálním souborovém systému na datových uzlech \cite{hadoopHortonworks}. O mapování bloků datovým uzlům se stará NameNode.
