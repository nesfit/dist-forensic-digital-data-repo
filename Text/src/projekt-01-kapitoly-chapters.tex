...
\noindent Tabulka \ref{tablePerformanceFinalSystem} udává naměřená data pro výsledný systém. Z tabulky, i porovnáním grafů \ref{FIG_PerformanceChartPrototype} a \ref{FIG_PerformanceChartFinalSystem}, lze vidět podstatné zvýšení výkonu. Pro takto malé PCAP soubory je už ve finálním systému většina času spotřebována komunikací klienta se systémem přes Kafku než parsováním paketů, získáváním metadat, a ukládáním metadat a hodnot paketů do databází. I přes přidaný podsystém metadat a předávání PCAP souborů přes HDFS bylo pomocí vylepšení uvedených v \ref{improvementsInImpl} docíleno zrychlení systému.
